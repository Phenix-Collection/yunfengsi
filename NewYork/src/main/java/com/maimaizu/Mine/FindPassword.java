...
public class FindPassword extends AppCompatActivity implements View.OnClickListener {
    ...
    private void submitEmail() {
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    String data = OkGo.post(Constants.email_findpsd__IP).params("email", phoneNum.getText().toString())
    ...
    private void submitPhone() {
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    String data = OkGo.post(Constants.WJMM_IP).params("phone", phoneNum.getText().toString())
                            .params("password", newWord.getText().toString()).params("key", Constants.safeKey)
                            .params("m_id",Constants.M_id).execute().body().string();
                    if (!data.equals("")) {
                        if (AnalyticalJSON.getHashMap(data) != null && "000".equals(AnalyticalJSON.getHashMap(data).get("code"))) {
                            runOnUiThread(new Runnable() {
