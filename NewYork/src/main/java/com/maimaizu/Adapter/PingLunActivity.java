                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        try {
                            ...
                            final String data = OkGo.post(Constants.little_zixun_pl_add_IP).params("user_id", sp.getString("user_id", "")).params("ct_id", pLId)
...
//                }
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        try {
                            String data1 = OkGo.post(Constants.PL_DZ_IP).params("user_id", sp.getString("user_id", ""))
                                    .params("comment_id", pLId).params("key", Constants.safeKey).execute().body().string();
                            if (!data1.equals("")) {
    ...
    private void getData() {
        ...
        OkGo.post(Constants.little_pl_huifu__IP).params("key", Constants.safeKey).params("comment_id", getIntent().getStringExtra("id"))
                .params("page", page).execute(new AbsCallback<ArrayList<HashMap<String ,String >>>() {
            @Override
