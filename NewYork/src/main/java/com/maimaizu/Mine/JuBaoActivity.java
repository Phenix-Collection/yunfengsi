...
public class JuBaoActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        title.setHint(mApplication.ST("请输入主题"));
                        ...
                        public void run() {
                            try {
                                String data = OkGo.post(Constants.Suggest_Ip).tag(TAG)
                                        .params("key", Constants.safeKey).params("user_id", PreferenceUtil.getUserIncetance(JuBaoActivity.this)
                                                .getString("user_id", "")).params("title", t).params("contents", tijiao).execute().body().string();
