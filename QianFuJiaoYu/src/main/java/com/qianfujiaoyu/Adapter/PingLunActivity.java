...
//                }
    ...
    private void getData() {
        ...
        JSONObject js=new JSONObject();
        try {
            js.put("comment_id", getIntent().getStringExtra("id"));
            js.put("page",page);
            js.put("m_id",Constants.M_id);
        } catch (JSONException e) {
            e.printStackTrace();
        ...
        ApisSeUtil.M m=ApisSeUtil.i(js);
        OkGo.post(Constants.little_pl_huifu__IP)
                .params("key",m.K())
                .params("msg",m.M()).execute(new AbsCallback<ArrayList<HashMap<String,String>>>() {
            @Override
            ...
            @Override
            public void onSuccess(ArrayList<HashMap<String, String>> Pllist, Call call, Response response) {
                if ((Pllist != null)) {
