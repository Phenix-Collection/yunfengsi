...
//        });
        ...
        ((TextView) findViewById(R.id.title_title)).setText(mApplication.ST("出价记录"));
    ...
    private static class MessageAdapter extends BaseQuickAdapter<HashMap<String, String>, BaseViewHolder> {
        ...
        protected void convert(BaseViewHolder holder, HashMap<String, String> map) {
            RTextView user   = holder.getView(R.id.user);
            TextView  status = holder.getView(R.id.status);
            TextView  price  = holder.getView(R.id.price);
            if (holder.getAdapterPosition() == 0) {
               price.setTextColor(Color.RED);
               user.setTextColorNormal(Color.RED);
               status.setTextColor(Color.RED);
                status.setText("领先");
            }else{
                status.setText("出局");
                price.setTextColor(Color.BLACK);
                user.setTextColorNormal(Color.BLACK);
                status.setTextColor(Color.BLACK);
            }
            user.setText(map.get("pet_name"));
            price.setText("￥"+map.get("money"));
    ...
    private void getHistory() {
        if (Network.HttpTest(this)) {
            ...
            LogUtil.e("获取出价记录：：" + js);
            OkGo.post(Constants.AuctionBidList)
                    .tag(this)
                    .params("key", m.K())
                    .params("msg", m.M())
