 ...
 */
public class nianfo_home_tab3 extends AppCompatActivity implements View.OnClickListener, SwipeRefreshLayout.OnRefreshListener, LoadMoreListView.OnLoadMore {
    private static final String TAG = "nianfo_home_tab3";
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        ((TextView) findViewById(R.id.sb)).setText(mApplication.ST("遍"));
        ((TextView) findViewById(R.id.sb2)).setText(mApplication.ST("持"));
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        String data1 = OkGo.post(Constants.nianfo_home_chizhou_Get_Ip).params("key", ApisSeUtil.getKey())
                                .params("msg",ApisSeUtil.getMsg(js)).tag(TAG).execute().body().string();
                        if (!data1.equals("")) {
                            list = AnalyticalJSON.getList(data1, "japad");
                            typelist = AnalyticalJSON.getList(data1, "japa");
                            if (null != AnalyticalJSON.getHashMap(data1)) {
                                totalNum = AnalyticalJSON.getHashMap(data1).get("num");
                            } else totalNum = "0";
                            if (list != null && handler != null) {
                                ...
                                handler.post(new Runnable() {
                                    ...
                                    public void run() {
                                        ...
                                        ((TextView) findViewById(R.id.nianfo_title)).setText(mApplication.ST("公共累计持咒" + NumUtils.getNumStr(totalNum) + "遍"));
                        ...
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                Toast.makeText(nianfo_home_tab3.this, mApplication.ST("加载失败，请检查网络连接"), Toast.LENGTH_SHORT).show();
                                if (swip.isRefreshing()) swip.setRefreshing(false);
    ...
    public void onClick(final View v) {
        switch (v.getId()) {
            ...
            case R.id.nianfo_home_tab1_type:
                ...
                if (typelist != null) {
                    ArrayAdapter adapter = new mArrayAdapter(this, R.layout.nianfo_home_dialog_item, R.id.nianfo_home_dialog_item_text, typelist, "持咒");
            ...
            case R.id.nianfo_home_tab1_commit:
                ...
                if (!type.getText().toString().trim().equals("") && !num.getText().toString().trim().equals("")
                    ...
                    ProgressUtil.show(nianfo_home_tab3.this,"",mApplication.ST("正在提交"));
                    new Thread(new Runnable() {
                        @Override
                        public void run() {
                            String data1 = null;
                            try {
                                ...
                                data1 = OkGo.post(Constants.nianfo_home_chizhou_Commit_Ip).tag(TAG)
                                        ...
                                        runOnUiThread(new Runnable() {
                                            ...
                                            public void run() {
                                                if(!"0".equals(m.get("yundousum"))){
                                                    YunDouAwardDialog.show(nianfo_home_tab3.this,"每日持咒",m.get("yundousum"));
                                                }
                                                ProgressUtil.dismiss();
                                                HashMap<String, String> map = new HashMap<>();
                                                map.put("ls_time", TimeUtils.getStrTime(System.currentTimeMillis() + ""));
                                                map.put("ja_name", type.getText().toString());
